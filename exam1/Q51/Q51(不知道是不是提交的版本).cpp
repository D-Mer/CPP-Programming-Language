...
void homework() {
	...
	for (int i = 0; i < Q; i++) {
		...
		case 3:
			...
			time = temp[1].substr(5, temp[0].length() - 6);
			timeRaw = split(time, "-");
		...
		case 4:
			...
			time = temp[1].substr(7, temp[0].length() - 8);
			timeRaw = split(time, "-");
			q->city = city;
			q->start_y = stoi(timeRaw[0]);
			q->start_m = stoi(timeRaw[1]);
			time = temp[2].substr(5, temp[0].length() - 6);
			timeRaw = split(time, "-");
			q->end_y = stoi(timeRaw[0]);
			q->end_m = stoi(timeRaw[1]);
			querys.push_back(*q);
	...
	for (size_t i = 0; i < querys.size(); i++) {
		...
		case 3:
			for (size_t j = 0; j < records.size(); j++) {
				r = records[j];
				if (r.year >= q->start_y && r.year<= q->end_y && r.month >= q->start_m && r.month <= q->end_m) {
					found = true;
					result += r.water;
				}
			}
